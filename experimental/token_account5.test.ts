...
`
  ...
  it("should deploy the TokenAccount contract", async () => {
    ...
    const sender = deployer.toPublicKey();
    const token1 = new TokenAccount(user1PublicKey);
    const token2 = new TokenAccount(user2PublicKey);
  ...
  it("should change the state and check the balance", async () => {
    ...
    for (let i = 0; i < NUMBER_ITERATIONS; i++) {
      ...
      const tokenId1 = zkTokenAccount1.token.id;
      const tokenId2 = zkTokenAccount2.token.id;
      console.log(
        "Iteration",
        i,
        "hasAccount1",
        ...
        tokenId2.toJSON(),
        "\ntoken1",
        token1.token.id.toJSON(),
        "token2",
        token2.token.id.toJSON()
      );
