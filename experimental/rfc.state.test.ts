...
const votingTransactions: VotingTransaction[] = [];
...
describe(`Parallel SmartContract proofs calculations`, () => {
  it(`should prepare data`, async () => {
    ...
    for (let i = 0; i < maxElements; i++) {
      ...
      votingTransactions.push(votingTransaction);
    }
  ...
  it(`should calculate states from string`, async () => {
    expect(deployer).not.toBeUndefined();
    expect(votingContract).not.toBeUndefined();
    if (deployer === undefined || votingContract === undefined) return;
    ...
    console.log(`Computing ${maxElements} states...`);
    console.time(`calculated ${maxElements} states`);

    for (let i = 0; i < maxElements; i++) {
      const args = JSON.parse(transactions[i]);
    ...
    console.timeEnd(`calculated ${maxElements} states`);
  });

  it(`should calculate states from ready data`, async () => {
    expect(deployer).not.toBeUndefined();
    expect(votingContract).not.toBeUndefined();
    if (deployer === undefined || votingContract === undefined) return;
    const sender = deployer.toPublicKey();
    ...
    console.log(`Computing ${maxElements} states from ready data...`);
    console.time(`calculated ${maxElements} states`);
      ...
      await Mina.transaction(
        {
          sender,
          fee: transactionFee,
          nonce: nonce + votingTransactions[i].id,
        },
        () => {
          zkApp.addVoteToMerkleTree(
            votingTransactions[i].oldRoot,
            votingTransactions[i].newRoot,
            votingTransactions[i].witness,
            votingTransactions[i].value
          );
        }
      );
    }
