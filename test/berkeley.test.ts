...
import { Mina, Field, Poseidon, PrivateKey, UInt64, PublicKey, fetchAccount } from 'o1js';
...
beforeAll(async () => {
  ...
  if( useFaucet) {
    ...
    let count = 0
    while(waitForTopup) {
      const balance = Number((await accountBalance(deployer.toPublicKey())).toBigInt());
      if( balance > 0 ) 
        waitForTopup = false
      else  {
  ...
  const balanceDeployer = Number((await accountBalance(deployer.toPublicKey())).toBigInt()) / 1e9;
  console.log("Balance of the Deployer is ", balanceDeployer.toLocaleString("en"))
